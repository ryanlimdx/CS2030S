...
class BusService {
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    ...
    return BusAPI.getBusStopsServedBy(serviceId).thenApply(busStopsServedBy -> {
      Scanner sc = new Scanner(busStopsServedBy);
  ...
  public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
    return getBusStops().thenApply(busStops -> busStops
        .stream()
